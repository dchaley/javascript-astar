  ...
  */
  search: function(graph, start, end, options) {
    ...
    while (openHeap.size() > 0) {
      ...
      [path, closestNode] = astar.visitNode(graph, openHeap, heuristic, end, closestNode, node);
      if (path) {
        return path;
      }
    }
  ...
  visitNode: function(graph, queue, heuristic, end, closestNode, node) {
    ...
    if (node === end) {
      // return path (or null) + closest node found
      return [pathTo(node), node];
    }
    ...
    for (var i = 0, il = neighbors.length; i < il; ++i) {
      ...
      var gScore = node.g + neighbor.getCost(node);
      ...
      if (!beenVisited || gScore < neighbor.g) {
        ...
        if (!beenVisited) {
          ...
          queue.rescoreElement(neighbor);
        }
      }
    }

    // return null (no path found) + closest node found
    return [null, closestNode];
  },
